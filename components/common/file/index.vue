    ...
    div(@click="$emit('click', $event)")
      ...
      .button.inline-flex
        ...
        commonButton(
          ...
          @blur="$emit('blur')")
          iconAttachment
          span прикрепите файл
    .files.flex.column(v-if="filesCount")
      a.file.flex.a-center(v-for="(file, i) in value" :key="i" href="#" @click.prevent="remove(file)") {{ file.name }}
        iconRemove
</template>
...
export default {
  ...
  methods: {
    ...
    remove (file) {
      const filesArray = Array.from(this.value)
      const indexToRemove = filesArray.indexOf(file)
