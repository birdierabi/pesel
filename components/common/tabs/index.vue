...
export default {
  ...
  props: {
    ...
    // Флаг использования хэша в урле
    hash: Boolean
  },
  data () {
    return {
  ...
  watch: {
    $route () {
      if (this.hash) {
        ...
        if (hash !== this.active && this.tabs.find(i => i.alias === hash)) {
          this.setActive(this.tabs.find(i => i.alias === hash))
        } else if (!hash) {
  ...
  methods: {
    ...
    setActive (tab) {
      ...
      this.active = tab.id
      if (this.hash) {
        window.location.hash = tab.alias
      }
    },
    findActive () {
      let active = null
